...
console.log('Starting MCP v1.1 test...');

// FastMCP Client Functions
async function createMCPClient() {
  const client = new Client(
    {
      name: 'test-client-v1.1',
      version: '1.1.0'
    },
    {
      capabilities: {}
...
// Start the servers
const servers = spawn('npm', ['run', 'start-v1.1'], {
  stdio: 'inherit',
  shell: true
});

// Wait 15 seconds then run tests
setTimeout(() => {
  console.log('\nStarting tests...');
  
  // Test with native client (with retry)
  setTimeout(async () => {
    let client;
    let hasError = false;
    ...
    try {
      ...
      console.log('✅ Connected to MCP server v1.1');
      
      // Test 1: List tools
      console.log('\n=== TOOLS LIST (Native Client v1.1) ===');
      const toolsList = await listToolsNative(client);
      console.log(JSON.stringify(toolsList, null, 2));
      
      // Test 2: Call tool with 3 parameters
      console.log('\n=== TOOL CALL (Native Client v1.1) ===');
      const toolResult = await callToolNative(client, 'sum', { a: 5, b: 10, c: 15 });
      console.log(JSON.stringify(toolResult, null, 2));
      
      console.log('\n✅ All v1.1 tests completed successfully!');
  ...
  }, 2000); // 2 seconds delay before tools/list
}, 15000); // 15 seconds total wait
